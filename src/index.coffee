...
module.exports = (robot) ->
  ...
  robot.respond /newrelic/i, (message) ->
    for server in robot.brain.data.newrelic.servers
      last_reported_at = moment(server.last_reported_at).fromNow()
      cpu = "CPU: #{(Math.round(server.summary.cpu))}%"
      diskIo = "DiskIO: #{(Math.round(server.summary.disk_io))}% Disk IO"
      memory = "Memory: #{server.summary.memory}% (#{humanize.filesize(server.summary.memory_used)} / #{humanize.filesize(server.summary.memory_total)})"
      fullestDisk = "DiskUsed: #{server.summary.fullest_disk}% (#{humanize.filesize(server.summary.fullest_disk_free)} free)"
      message.send """ (#{server.name}) 
                       #{cpu}, 
                       #{diskIo},
                       #{memory},
                       #{fullestDisk}
                       #{last_reported_at}
                       [https://rpm.newrelic.com/accounts/#{accountId}/servers/#{server.id}]
                       - - - - - - - - - - 
                   """
